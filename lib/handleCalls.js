...
export const handleAnswerCall = async (session) => {
    ...
    try {
        ...
        // Acquire microphone with common audio processing constraints
        let localStream;
        try {
            ...
            console.error('Error accessing microphone when answering:', mediaError);
            throw new Error('Microphone access denied or not available');
        }

        // Accept the call and explicitly provide the local audio stream
        await session.accept({
            ...
            sessionDescriptionHandlerModifiers: [
                (description) => {
                    // Force audio to be sendrecv to ensure two-way media
...
export const handleEndCall = async (session, sipUserAgent) => {
    ...
    try {
        // Send proper SIP signaling based on session state
        try {
            ...
            console.warn('Error sending BYE/CANCEL/REJECT:', signalError);
        }

        // Stop ringing sound if any
        try { await stopRingingSound(); } catch (_) {}

        // Ensure media streams and peer connection are cleaned up locally
        try {
            ...
            if (session._localStream) {
                session._localStream.getTracks().forEach(t => t.stop());
            }
        } catch (cleanupError) {
            console.warn('Error during media cleanup:', cleanupError);
        }
